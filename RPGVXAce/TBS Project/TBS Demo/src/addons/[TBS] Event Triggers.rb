...
#     new attributes: battler, move_type, team
...
#                  draw_skill_cost(rect,skill,enabled)
#
#   New classes/modules:
#   module TBS
...
class Game_Battler < Game_BattlerBase
  ...
  def can_cross_ev?(moveRule,dir,event)
    ...
    other_move_type = event.move_type
...
class Game_Event < Game_Character
  ...
  def read_triggers
    ...
    @move_type = tbs_trigger.move_type
...
class Window_TBS_ActorCommand < Window_ActorCommand
  ...
  def add_event_skill_command(skill_id)
    ...
    add_command_tbs(skill.name, :event_skill, @actor.usable?(skill) && @actor.current_action, skill_id)
  end
