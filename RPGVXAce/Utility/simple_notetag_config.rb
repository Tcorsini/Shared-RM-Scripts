...
# DATA can be DATA_SKILLS, DATA_ITEMS, DATA_WEAPONS, DATA_ARMORS,
...
# For DATA_TROOPS, the data will be read from comments in every page of the troop 
...
#
# symb    is the name of the attribute added to the database, it will be set as an accessor,
#         for instance: symb=:range means that obj.range will be accessible from the database
# default is the value set by default when no matching line to the regexp was found
# regexp  is a regular expression set to find the value, it will be interpreted with type
# type    is an integer related to AssignationModes indices
#         0/none provided -> the data is read and evaluated (int, bool, float, symbols, arrays)
#         1               -> the string is stored as is (files, texts)
#         2               -> will replace the default value by true if the regexp is found
#
# You can put a Notetag_Data object x to multiple arrays by calling x.add_to(a1,a2,...)
#
# Example:
...
# If no such notetag was given, the default value will be stored instead (here 1)
#
...
# in RPG::BaseItem, RPG::MapInfo, RPG::Troop and their subclasses
#============================================================================
...
module SNC
  ...
  def self.prepare_metadata
    #You can either redefine the arrays like this:
    # DATA_SKILLS = [
    ...
    # ]
    #
    #Or you can add the Notetag_Data objects like this:
    # DATA_ITEMS += [obj1,obj2]
    #
    #Finally, you can also add the same notetag object to multiple data arrays:
...
#============================================================================
  ...
  def self.load_notetags_metadata(obj,data_list,note)
    ...
    note.split(/[\r\n]+/).each { |line|
      data_list.each do |d|
        line.scan(d.regexp)
        obj.instance_variable_set(d.attribute_name,d.value($1)) if $1
      end
    } # self.note.split
...
class Notetag_Data
  ...
  AssignationModes = [:eval, :string, :activate]
  #accessors:
  ...
  #--------------------------------------------------------------------------
  # method: value -> given a string identified by the regexp, 
  # converts it into a value to store
  #--------------------------------------------------------------------------
  def value(str)
